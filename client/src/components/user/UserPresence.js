...
function UserPresence() {
  ...
  return (
    <div className="p-2 bg-dark-800 border-t border-dark-600">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="relative">
            ...
            {/* Status indicator */}
            <div className={`absolute bottom-0 right-0 w-3 h-3 ${currentStatusOption.color} rounded-full border-2 border-dark-800`} />
          </div>
      ...
      {/* Status menu */}
      {showStatusMenu && (
        <div className="absolute bottom-16 left-2 bg-dark-700 rounded-lg shadow-lg py-2 z-50">
          {statusOptions.map(option => (
            <button
              key={option.value}
              onClick={() => handleStatusChange(option.value)}
              className="w-full px-4 py-2 flex items-center space-x-3 hover:bg-dark-600 transition"
            >
          ...
          <div className="border-t border-dark-600 mt-2 pt-2 px-4">
            <input
              type="text"
              placeholder="Set custom status..."
              value={customStatus}
              onChange={(e) => setCustomStatus(e.target.value)}
              className="w-full bg-dark-600 text-white text-sm px-2 py-1 rounded"
            />
          </div>
        </div>
      )}
